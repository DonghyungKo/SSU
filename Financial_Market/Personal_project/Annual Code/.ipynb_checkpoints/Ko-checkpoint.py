...
def portfolio_selection(df):
    ...
    for cnt, month in enumerate(month_list):        
        ...
        if not cnt == len(month_list) - 1:
            ...
            quantile_value_list += [np.percentile(temp_t.values, i*10)for i in range(1,11)]
            
            for i, Q in enumerate(quantile_value_list):
                if not i == 10:
                    df_temp = temp_t[temp_t > Q] <= quantile_value_list[i+1]
